...
def sort_by_alphabet(notes: Notebook):
    ...
    for i in notes.data.values():
        title_list.append(copy(i.note[0].title.split()))
    for j in title_list:
        title_string = " ".join(j)
        for val in default_notebook.data.values():
            if title_string in val.note[0].title:
                sorted_notes.add(val)
    display(sorted_notes)
